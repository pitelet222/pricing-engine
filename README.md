...
│   │   ├── routes.py             # All endpoint handlers (7 routes)
...
│
├── tests/                        # Pytest test suite (252 unit + 28 integration)
├── .github/workflows/ci.yml      # GitHub Actions CI (5 parallel jobs)
├── Makefile                      # Developer targets — test, lint, api, dashboard…
├── scripts/
...
The REST API exposes seven endpoints. Full interactive docs at `/docs` when running.
...
```bash
curl http://localhost:8000/uncertainty/Chicago_conventional
```
...
```bash
# Fast unit tests — no model artefacts required, runs in CI (252 tests)
make test
...
# lightweight LightGBM on synthetic data; requires lightgbm + scikit-learn (28 tests)
