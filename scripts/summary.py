...
for t, name in [(0, "Conventional"), (1, "Organic")]:
    ...
    print(
        f"  {name}: mean=${g.mean():.2f}  std={g.std():.2f}  min=${g.min():.2f}  max=${g.max():.2f}"
    )
...
for t, name in [(0, "Conventional"), (1, "Organic")]:
    g = r[r["is_organic"] == t]
    print(
        f"  {name}: median_uplift={g['revenue_change_pct'].median():.1f}%  median_price_change={g['price_change_pct'].median():.1f}%"
    )

e = pd.read_csv(P + "price_elasticity.csv")
print("\n=== MOST ELASTIC (top 5) ===")
print(
    e.nsmallest(5, "elasticity")[["unique_id", "AveragePrice", "elasticity"]].to_string(index=False)
)
print("\n=== LEAST ELASTIC / PRICING POWER (top 5) ===")
print(
    e.nlargest(5, "elasticity")[["unique_id", "AveragePrice", "elasticity"]].to_string(index=False)
)

u = pd.read_csv(P + "uncertainty_pricing.csv")
print("\n=== HIGHEST REVENUE SPREAD (most uncertain) ===")
print(
    u.nlargest(5, "rev_spread_pct")[
        ["unique_id", "rev_spread_pct", "downside_risk_pct", "uplift_sharpe"]
    ].to_string(index=False)
)
print("\n=== BEST SHARPE (highest quality opportunities) ===")
print(
    u.nlargest(5, "uplift_sharpe")[
        ["unique_id", "opt_price_balanced", "rev_uplift_balanced", "uplift_sharpe"]
    ].to_string(index=False)
)
