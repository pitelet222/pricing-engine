...
class ForecastPoint(BaseModel):
    ds: date
    ensemble_weighted: float = Field(
        description="Ensemble (inverse-MAE weighted) point forecast (USD)"
    )
    ...
    nbeatsx: float | None = Field(
        default=None, description="NBEATSx neural forecast (None if not computed)"
    )
    dlinear: float | None = Field(
        default=None, description="DLinear neural forecast (None if not computed)"
    )
...
class RecommendationResponse(BaseModel):
    ...
    price_change_pct: float = Field(description="(optimal - current) / current × 100")
            ...
            "examples": [
                {
                    ...
                    "drivers": [
                        ...
                        {
                            "driver_rank": 2,
                            "feature": "region_volume_level",
                            "shap_value": -1.083,
                            "abs_shap_value": 1.083,
                            "direction": "decreases_demand",
                            "feature_value": 10.742,
                        },
                        {
                            "driver_rank": 3,
                            "feature": "plu_share_4770",
                            "shap_value": 0.207,
                            "abs_shap_value": 0.207,
                            "direction": "increases_demand",
                            "feature_value": 0.000438,
                        },
                    ],
                }
            ]
...
class PricingStrategy(BaseModel):
    ...
    rev_uplift_pct: float = Field(description="Expected revenue uplift vs current price (%)")
...
class UncertaintyResponse(BaseModel):
    ...
    is_organic: bool
    current_price: float = Field(description="Most recent actual price (USD)")
    conservative: PricingStrategy = Field(description="Low-risk strategy: modest price adjustment")
    balanced: PricingStrategy = Field(description="Balanced risk/return strategy")
    aggressive: PricingStrategy = Field(description="High-return strategy: larger price move")
...
class SimulateRequest(BaseModel):
    ...
    model_config = {
        "json_schema_extra": {"examples": [{"unique_id": "Albany_conventional", "price": 1.50}]}
    }
...
class SimulateResponse(BaseModel):
    ...
    current_price: float = Field(description="Most recent actual price for this series")
    current_volume: float = Field(description="Most recent actual total volume for this series")
